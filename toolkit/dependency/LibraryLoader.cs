...
{
    private readonly Dictionary<string, IModule[]> _knownModules = new();
    public void Load(ILibrary library)
    {
        Dictionary<string, IModule[]> loadedModules =
            library.GetModules().GroupBy(x => x.Namespace).ToDictionary(x => x.Key, x => x.ToArray());

        foreach (KeyValuePair<string, IModule[]> type in loadedModules)
        {
            if (this._knownModules.TryGetValue(type.Key, out IModule[]? module))
            {
                throw new LibraryNamespaceConflictException(type.Key, type.Value.First(), module.First());
            }
