...
public class FieldMember : Member
{
    ...
    public FieldMember(string name, CarpType type, CarpObject value) : base(name, type)
    {
    ...
    public override CarpObject Set(CarpObject? self, CarpObject value)
    {
        if (this.Is(Modifiers.Final))
            throw new InvalidAssignmentTargetException($"Field '{this.Name}' is read-only");
    ...
    public override Member Clone()
        =>
            ...
            new FieldMember(this.Name, this.Type, this._value);
}